...
public class OutboundClient extends Client {
    ...
    public OutboundClient() {
        this.enquireLinkExecutor = Executors.newScheduledThreadPool(1, r -> {
    ...
    protected synchronized void reconnect(Integer connectionFailedTimes) {
        ...
        if (!getConnectionFailedTimes().equals(connectionFailedTimes)) {
            logger.info("skipping reconnect for client {} due to optimistic lock", this, connectionFailedTimes, getConnectionFailedTimes());
            return;
        }
    ...
    private void runEnquireLinkTask() {
        Integer enquireLinkPeriod = 10000;
        Integer enquireLinkTimeout = 10000;
        enquireLinkTask = this.enquireLinkExecutor.scheduleWithFixedDelay(new EnquireLinkTask(this, enquireLinkTimeout), enquireLinkPeriod, enquireLinkPeriod, TimeUnit.MILLISECONDS);
    }
